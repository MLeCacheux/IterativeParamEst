...
def model_function(condition, time, timestep):
    AC_start, trigger_start, FI_start, SI_start, sv = condition.iloc[0]['AC_start'], condition.iloc[0]['trig_start'], condition.iloc[0]['FI_start'], condition.iloc[0]["SI_start"], condition.iloc[0]["sv"]
    rate_const = {'kI': 0.0449, 'kII': 0.505, 'kIII': 0.00370, 'kIV': 500,  'flow': sv} #input reaction rate constants
...
# Determines if the functino oscillates
...
#Determines whether the oscillating functions exhibits sustained oscillations, prints its related properties, period and amplitude
...
conditions_table = "Large_space_conditions.csv" #name of the conditions table to screen, the file should be made using the conditions_maker for model screening.py Python script
...
timestep = 10
folder = os.getcwd() + "//"
file_name = "Model screening results.csv" #output file, can be read using interactive_analysis.py
